...
export class AuthController {
  ...
  @Get('/register-guest')
  async registerAccount(@Res() response: Response, @Req() request: Request): Promise<any> {
    try {
      const hostBE: string = request.get('host');
      const userInformation: UserInterface = await this.userService.generateGuest(hostBE);
  ...
  async handleGoogleLogin(@Body('token') token: string, @IdUser() userId: number, @Res() response: Response) {
    try {
  ...
  @UseGuards(AuthorizeJWT)
  @Post('/logout')
  async handleLogout(@Res() response: Response, @IdUser() idUser: number) {
    try {
