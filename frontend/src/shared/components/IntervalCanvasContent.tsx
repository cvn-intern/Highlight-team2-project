...
const IntervalCanvasContent = ({ status = INTERVAL_SHOW_WORD }) => {
  ...
  const correctPlayers = useMemo(() => {
    return participants.filter((participant) =>
      correctAnswers.includes(participant.id)
    );
  }, [correctAnswers, participants]);
    ...
    case END_GAME:
      return (
        <>
          ...
          <div
            className={cn(
              'flex flex-row justify-center items-center gap-y-3 h-60 mt-3',
              {
          ...
          >
            <div className="flex flex-row w-4/5 h-4/5 justify-around">
              {participants &&
                handleTop3Ranking(participants).map(
                    ...
                    >
                      <div className="flex flex-col justify-center items-center h-full">
                        <div className="flex flex-row justify-center">
                          ...
                          <Avatar
                            className={cn(
                              'relative flex items-center h-auto group-hover:scale-110 overflow-visible border-4 border-black border-solid',
                              {
                                'w-[60%]': item.is_winner,
                        ...
                        <p
                          className={cn('text-2xl text-slate-800', {
